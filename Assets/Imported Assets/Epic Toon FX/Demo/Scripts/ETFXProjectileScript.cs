...
public class ETFXProjectileScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            transform.rotation = Quaternion.LookRotation(myRigidBody.velocity); // Sets rotation to look at direction of movement
        }
        ...
        Vector3 direction = myRigidBody.velocity; // Gets the direction of the projectile, used for collision detection
        if (myRigidBody.useGravity)
            direction += Physics.gravity * Time.deltaTime; // Accounts for gravity if enabled
        direction = direction.normalized;

        float detectionDistance = myRigidBody.velocity.magnitude * Time.deltaTime; // Distance of collision detection for this frame
    ...
    public void OnDestinationReached()
    {
        ...
        Destroy(projectileParticle, 3f); // Removes particle effect after delay
        Destroy(impactP, 3.5f); // Removes impact effect
        Destroy(gameObject, 10); // Removes the projectile
    }
